...
export function PlaystyleSelect({ onSelect }: PlaystyleSelectProps) {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/55 backdrop-blur-sm">
      <div className="w-full max-w-md mx-4 rounded-3xl bg-gray-900/85 backdrop-blur-md border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.7)] px-8 py-10">
        ...
        <div className="grid grid-cols-2 gap-4 mt-8">
          {OPTIONS.map(({ ps, icon, name, desc }) => (
            <button
              key={ps}
              onClick={() => choose(ps)}
              className="flex flex-col items-center gap-3 p-5 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 hover:border-summer-coral/50 active:scale-95 transition-all duration-200 text-center focus:outline-none focus:ring-2 focus:ring-summer-coral/50"
            >
